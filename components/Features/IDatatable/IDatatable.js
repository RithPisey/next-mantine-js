...
CommonDataTable.Filters = function ({
	...
	const hideColumnOptions = filterData.hideColumns.map((item) => (
		<Combobox.Option value={item.field} key={Date.now() + item.field}>
			<Group>
				{item.hideable && translate(item.header)}{" "}
				{filterData.values.hideColumn.some((hcol) => {
			...
			return (
				<Table.Td ta={col.field === "irow_index" && "center"}>
					{col.field === "irow_index" ? (
						...
						<col.renderCell value={row[col.field]} row={row} />
					) : (
						row[col.field]
					)}
				</Table.Td>
...
function CommonDataTable({
	...
	const [filterData, setFilterData] = useState({
		...
		values: {
			rowPerPage: 5,
			sortBy: columns.find((item) => item.defaultSortby)?.field,
			sortType: "asc",
			hideColumn: [],
			page: currentPage,
		},
	...
	useEffect(() => {
		onFilterChange(filterData.values);
	}, []);
	useEffect(() => {
		// console.log(context);
		setFilterData((prev) => ({
